...
if 'Windows' in platform.platform():
    ...
    BASE_DATASET = "./data/PlacePulse"

CATEGORIES = ('beautiful', 'boring', 'depressing', 'lively', 'safety',
              'wealthy')
...
class PlacePulseDataset(BaseSiameseDataset):
    def __init__(self,
        ...
        if len(kwargs) > 0:
            print(f"Unexpected args for PlacePulseDataset: {kwargs}")
        self._split = split
        self._category_id = category_id
        self._include_equal = include_equal
        self._dataset_dir = dataset_dir
        self._image_dir_name = image_dir_name
        super().__init__(split, category_id, trans_config)
    ...
    def _get_list_and_labels(self, split):
        ...
        pair_list = [(os.path.join(image_dir, left_id[i] + '.jpg'),
...
class PlacePulsePredictDataset(BasePredictDataset):
    def __init__(
        ...
        super().__init__(min_height, min_width, is_bgr)
    ...
    def _get_image_full_paths(self):
        ...
        if self._broken_file_name is not None:
            with open(os.path.join(self._dataset_dir, self._broken_file_name),
                      'r') as f:
                broken_files = f.readlines()
            broken_files = [name.replace('\n', '') for name in broken_files]
        image_list = [
            os.path.join(image_dir, fname) for fname in file_names
