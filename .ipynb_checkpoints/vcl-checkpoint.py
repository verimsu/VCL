...
class vloss(nn.Module):
    ...
    def jsd_gaussian(self, p_mean, q_mean, m_mean, logVar_):
        logVarp = logVar_[:,0,:]
        logVarq = logVar_[:,1,:]
        
        std = 0.5 * ((logVarp /2).exp() + (logVarq /2).exp())
        logVarm = std.log() * 2
        
        term_1 = 0.5 * ((logVarm - logVarq) + (logVarm - logVarq))
        term_2 = ((m_mean - p_mean).pow(2) + (m_mean - q_mean).pow(2)).div(2 * logVarm.exp())
    ...
    def forward(self, mu, logVar):
        ...
        jsd = self.jsd_gaussian(mp, mq, mm, logVar).mean()
